...
class AppXRTE:
    def __init__(self, master):
        ...
        webedit.add_command(
            label="Import XML", command=self.AppXRWEOpen, accelerator="Ctrl+I"
        )
        master.bind("<Control-Key-I>", self.AppXRWEOpen)
        webedit.add_command(
            label="Export XML...", command=self.AppXRWESave_as, accelerator="Ctrl+X"
        )
        ...
        helpmenu.add_command(
            label="About XRead", command=self.About, accelerator="Ctrl+A"
        )
        master.bind("<Control-Key-A>", self.About)
        helpmenu.add_command(
            label="About Web Editor", command=self.AppXRWEAbout, accelerator="Ctrl+W"
        )
        master.bind("<Control-Key-W>", self.AppXRWEAbout)
        helpmenu.add_command(
            label="Web Editor Style",
            command=self.AppXRWEStyle,
